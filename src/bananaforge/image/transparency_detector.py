...
class TransparencyDetector:
    ...
    def _analyze_png_transparency(self, image: Image.Image) -> TransparencyInfo:
        ...
        if image_mode in ("RGBA", "LA"):
            has_transparency, transparent_count, details = self._analyze_alpha_channel(
                image
            )
            if has_transparency:
        ...
        # Other modes
            ...
            detection_details.append(
                f"Mode {image_mode} - transparency support unknown"
            )

        # Calculate transparency percentage
        transparency_percentage = (
            (transparent_pixel_count / total_pixels * 100) if total_pixels > 0 else 0.0
        )
    ...
    def _analyze_gif_transparency(self, image: Image.Image) -> TransparencyInfo:
        ...
        if is_animated:
            detection_details.append(
                f"animated GIF detected with {getattr(image, 'n_frames', 1)} frames"
            )
        ...
        if image_mode == "P" and "transparency" in image.info:
            ...
            # Count transparent pixels
            transparent_pixel_count = self._count_gif_transparent_pixels(
                image, transparency_info
            )
            opaque_pixel_count = total_pixels - transparent_pixel_count

            if is_animated:
                detection_details.append(
                    "transparency analysis based on first frame of animation"
                )
            ...
            opaque_pixel_count = total_pixels
            detection_details.append(
                "GIF in palette mode but no transparency index found"
            )
            ...
            opaque_pixel_count = total_pixels
            detection_details.append(
                f"GIF in {image_mode} mode - no transparency support"
            )
    ...
    def _count_gif_transparent_pixels(
        self, image: Image.Image, transparency_index: int
    ) -> int:
        """
