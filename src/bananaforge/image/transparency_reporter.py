...
class TransparencyStatistics:
    ...
    transparency_distribution: (
        str  # "scattered", "clustered", "edge-focused", "uniform"
    )
...
class TransparencyReporter:
    ...
    def generate_detailed_report(
        self, transparency_info: TransparencyInfo, image_path: str
    ) -> TransparencyStatistics:
        """
    ...
    def _create_no_transparency_stats(
        self, info: TransparencyInfo
    ) -> TransparencyStatistics:
    ...
    def _analyze_transparency_patterns(
        self, image: Image.Image, info: TransparencyInfo
    ) -> TransparencyStatistics:
        ...
        return TransparencyStatistics(
            ...
            largest_transparent_region_size=max(
                (region["size"] for region in transparency_regions), default=0
            ),
    ...
    def _calculate_alpha_statistics(
        self, alpha_channel: np.ndarray
    ) -> Dict[str, float]:
    ...
    def _analyze_transparency_regions(
        self, alpha_channel: np.ndarray
    ) -> List[Dict[str, any]]:
        ...
        def flood_fill(start_y, start_x):
            ...
            if (
                ...
                or not transparent_mask[start_y, start_x]
            ):
                return 0
    ...
    def _classify_transparency_distribution(
        self, alpha_channel: np.ndarray, regions: List[Dict]
    ) -> str:
    ...
    def _count_edge_transparency(self, alpha_channel: np.ndarray) -> int:
        ...
        height, width = alpha_channel.shape
        edge_width = min(
            10, min(height, width) // 10
        )  # 10px or 10% of smallest dimension
    ...
    def _generate_recommendations(
        ...
        if info.transparency_percentage > 75:
            issues.append(
                "High transparency percentage may indicate background removal needed"
            )
        ...
        if distribution_pattern == "edge-focused":
            issues.append(
                "Edge transparency detected - likely anti-aliasing or feathering"
            )
    ...
    def format_statistics_report(
        self, stats: TransparencyStatistics, verbose: bool = False
    ) -> str:
        """
        ...
        if stats.transparency_percentage > 0:
            ...
            lines.append(
                f"  • Mean alpha: {stats.alpha_distribution_summary['mean']:.1f}"
            )
            lines.append(
                f"  • Alpha range: {stats.alpha_distribution_summary['min']:.0f} - {stats.alpha_distribution_summary['max']:.0f}"
            )
            lines.append(
                f"  • Standard deviation: {stats.alpha_distribution_summary['std']:.1f}"
            )
        ...
        if stats.transparency_regions:
            ...
            lines.append(
                f"  • Largest region: {stats.largest_transparent_region_size:,} pixels"
            )
            if verbose and len(stats.transparency_regions) > 0:
                ...
                for i, region in enumerate(stats.transparency_regions[:3]):
                    lines.append(
                        f"    {i+1}. Size: {region['size']:,} pixels at ({region['center_x']}, {region['center_y']})"
                    )
            lines.append("")
        ...
        lines.append(
            f"  • Complexity score: {stats.complexity_score:.2f} (0=simple, 1=complex)"
        )
        lines.append(
            f"  • Recommendation: {stats.processing_recommendation.replace('_', ' ').title()}"
        )
        lines.append("")
        ...
        if verbose and stats.transparency_percentage > 0:
            ...
            if stats.alpha_histogram:
                ...
                sorted_histogram = sorted(
                    stats.alpha_histogram.items(), key=lambda x: x[1], reverse=True
                )
                for alpha_val, count in sorted_histogram[:10]:
                    percentage = (count / stats.total_pixels) * 100
                    lines.append(
                        f"    Alpha {alpha_val}: {count:,} pixels ({percentage:.1f}%)"
                    )
            lines.append("")
