...
class ModelExporter:
    ...
    @staticmethod
    def estimate_triangle_count(height: int, width: int) -> int:
        """Estimate triangles generated by the heightmap mesh."""
        if height < 2 or width < 2:
            return 0

        surface_triangles = 4 * (height - 1) * (width - 1)
    ...
    def fit_shape_to_triangle_limit(
        ...
        while (
            cls.estimate_triangle_count(target_height, target_width) > max_triangles
            and target_height > 2
            and target_width > 2
        ):
    ...
    def _apply_triangle_limit(
        ...
        estimated_triangles = self.estimate_triangle_count(height, width)
        if estimated_triangles <= max_triangles:
            return height_map, material_assignments

        target_height, target_width = self.fit_shape_to_triangle_limit(
            height, width, max_triangles
        )
        target_triangles = self.estimate_triangle_count(target_height, target_width)
